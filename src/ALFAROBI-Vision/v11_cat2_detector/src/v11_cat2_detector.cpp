...
cv::Mat Cat2Detector::segmentColor(cv::Mat &_segmented_green, cv::Mat &_inv_segmented_green, cv::Mat &_segmented_blue, cv::Mat &_segmented_white){
    ...
    //FSM BUAT ROBI kalo worst case lokalisasi ga kelar
    //masuk lapagan
    //maju selama X detik sampai lokasi yg diinginkan
    //rotate ke arah gawang musuh
    //cari bola
    //jarak 150 kejar
    //tendang
    //abis tendang pokoknya kamera harus nunduk biar ga langsung detek garis horizontal
    //rotate
    //maju
    //if garis found
    //rotate lagi
    //kembali ke cari bola
    //s)p ajus gay
...
//Tiang Baru
    geometry_msgs::Point _pillar_pos_;
    ...
    cv::Mat masklap = segmented_green;
    // cv::imshow("masklap",masklap);
    ...
    cv::Mat maskgar = segmented_white;
...
//            cv::Point br_pt = rect_rough_roi.br();
    ...
    cv::bitwise_and(inv_segmented_green,field_for_line,inv_segmented_green);
    // cv::imshow("field_for_line",field_for_line);
    // cv::waitKey(1);
    // cv::imshow("inv_segmented_green",inv_segmented_green);
...
// #### Vertical Line
    ...
    for(cv::Vec4i reduced: reducedLines){
        ...
        cv::Point p1, p2;
            ...
            //horizontal lines
                ...
                // else if(angle >= 75){
                //     tiang.push_back(reduced);
                // }

        //circle(frameclone, Point(x,y),3,Scalar(255,0,0),-1,8,0);
    }
    //GAMBAR TIANG
    for(size_t i = 0; i<tiang.size(); i++){
        cv::Vec4i tiang_garis = tiang[0];
        //untuk tiang
        _pillar_pos_.x = int((tiang_garis[2]+tiang_garis[0])/2);
        _pillar_pos_.y = int((tiang_garis[3]+tiang_garis[1])/2);

        double center_x_ver = (tiang_garis[2]+tiang_garis[0])/2;
        double center_y_ver = (tiang_garis[3]+tiang_garis[1])/2;

        point1=cv::Point(tiang_garis[0], tiang_garis[1]);
        point2=cv::Point(tiang_garis[2], tiang_garis[3]);
        cv::line(output_view, point1, point2, cv::Scalar(255,0,255), 4, cv::LINE_AA);
        cv::circle(output_view, cv::Point(center_x_ver,center_y_ver),6,cv::Scalar(0,0,0),-1,8,0);
        //UNTUK TIANG
        if(!tiang.size()){
            pillar_pos_.x = -1;
            pillar_pos_.y = -1;
            ...
            pillar_pos_.x = _pillar_pos_.x;
            pillar_pos_.y = _pillar_pos_.y;
            pillar_pos_.z = 0;
        }
        std::cout<<"pillar_pos_.x : "<<pillar_pos_.x<<std::endl;
        std::cout<<"pillar_pos_.y : "<<pillar_pos_.y<<std::endl;
        if(i==0){
            break;
        }  
    ...
    for( size_t i = 0; i < garis_ver.size(); i++ ){
        cv::Vec4i garis_vertikal = garis_ver[0]; 
    // cv::Point point1, point2;
    //titik tengah yg di publish
    ...
    // else{
        ...
        //untuk gambar
        double center_x_ver = (garis_vertikal[2]+garis_vertikal[0])/2;
        double center_y_ver = (garis_vertikal[3]+garis_vertikal[1])/2;

        point1=cv::Point(garis_vertikal[0], garis_vertikal[1]);
        point2=cv::Point(garis_vertikal[2], garis_vertikal[3]);
        cv::line(output_view, point1, point2, cv::Scalar(0,0,255), 4, cv::LINE_AA);
        cv::circle(output_view, cv::Point(center_x_ver,center_y_ver),6,cv::Scalar(0,0,0),-1,8,0);
    // }
     if(!garis_ver.size()){
            line_pos_ver_.x = -1;
    ...
    std::cout<<"line_pos_ver_.x : "<<line_pos_ver_.x<<std::endl;
        std::cout<<"line_pos_ver_.y : "<<line_pos_ver_.y<<std::endl;
     if(i==0){
            break;
        }  
        
    }
        //Gambar maksimal 1 garis HORIZONTAL tiap frame
    for( size_t i = 0; i < garis_hor.size(); i++ ){
        cv::Vec4i garis_horizontal = garis_hor[0]; 
    // cv::Point point1, point2;
    //titik tengah yg di publish
    // _line_pos_ver.x = int((garis_horizontal[2]+garis_horizontal[0])/2);
    // _line_pos_ver.y = int((garis_horizontal[3]+garis_horizontal[1])/2);
    // if(garis_hor.empty()){
    //     continue;
    // } else{
        //PUBLISHER
        ...
        //TITIK TENGAH BUAT GAMBAR
        double center_x_hor = (garis_horizontal[2]+garis_horizontal[0])/2;
        double center_y_hor = (garis_horizontal[3]+garis_horizontal[1])/2;
        ...
        cv::circle(output_view, cv::Point(center_x_hor,center_y_hor),6,cv::Scalar(0,255,0),-1,8,0);
    // }
    if(!garis_hor.size()){
            line_pos_.x = -1;
            line_pos_.y = -1;
...
//tiang baru
    ...
    //cv::waitKey(27);
    cv::imshow("horizontal dan vertikal",output_view);
    // cv::imshow("vertikal",output_view);
