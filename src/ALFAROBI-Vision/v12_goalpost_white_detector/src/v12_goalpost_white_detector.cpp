#include "v12_goalpost_white_detector/v12_goalpost_white_detector.h"
...
GoalpostDetector::GoalpostDetector()
    ...
    nh_.param<std::string>("goalpost_config_path", goalpost_config_path,
                           ros::package::getPath("v12_goalpost_white_detector") + "/config/saved_config.yaml");

    param_cb_ = boost::bind(&GoalpostDetector::paramCallback, this, _1, _2);
    param_server_.setCallback(param_cb_);

    LUT_dir = ros::package::getPath("v12_goalpost_white_detector") + "/config/tabel_warna.xml";
...
void GoalpostDetector::loadParam(){
    ...
    try{
        ...
        ROS_ERROR("[v12_goalpost_white_detector] Unable to open config file: %s", e.what());
    }
    ...
    goalpost_ref_ = cv::imread(ros::package::getPath("v12_goalpost_white_detector") + "/config/goalpost_ref.jpg");
...
void GoalpostDetector::saveParam(){
    ...
    cv::imwrite(ros::package::getPath("v12_goalpost_white_detector") + "/config/goalpost_ref.jpg", goalpost_ref_);
}
...
void GoalpostDetector::imageCallback(const sensor_msgs::ImageConstPtr &_msg){
    ...
    try{
        ...
        ROS_ERROR("[v12_goalpost_white_detector] cv bridge exception: %s",e.what());
        return;
    }
...
void GoalpostDetector::paramCallback(v12_goalpost_white_detector::GoalpostDetectorParamsConfig &_config, uint32_t level){
    (void)level;
...
float GoalpostDetector::checkTargetHistogram(cv::Mat _target_roi){
    ...
    if(goalpost_ref_.empty()){
        ROS_ERROR("[v12_goalpost_white_detector] Goalpost reference not found !!!");
        return -1;
    }
...
void GoalpostDetector::process(){
    ...
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(goalpost_in_background, contours, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);
    cv::Mat image_con = cv::Mat::zeros(goalpost_in_background.size(),CV_8UC3);
    ...
    if(contours.size()){
        for(size_t h = 0; h < contours.size(); h++){
            if(cv::contourArea(contours[h]) > 1000)
            contours_hull.insert(contours_hull.end(), contours[h].begin(), contours[h].end());
        }
    }

    if(contours_hull.size()){
        cv::convexHull(contours_hull, hull[0]);
        cv::drawContours(image_con, hull, 0, color, cv::FILLED);
    }

    cv::Mat image_bg, image_hsv_bg, image_thres_bg;
    cv::bitwise_and(in_img_, image_con, image_bg);

    cv::cvtColor(image_bg, image_hsv_bg, cv::COLOR_BGR2HSV);
    cv::inRange(image_hsv_bg, cv::Scalar(0,0,235), cv::Scalar(255,255,255), image_thres_bg);

    cv::Mat ver = image_thres_bg.clone();
    int hor_size = ver.cols/80;
    int ver_size = ver.rows/40;
    cv::Mat ver_struktur = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(hor_size, ver_size));
    ...
    {
        cv::Vec4i l = linesP[i];
        cv::line( garis, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), cv::Scalar(255,0,0), 1, cv::LINE_AA);
        ...
        cv::circle(output_view, center_rect, 5, cv::Scalar(0,0,255));
                ...
                if(!linesP.size()){
                    ...
                    // goalpost_pos_.x = point_x;
                    // goalpost_pos_.y = point_y;
