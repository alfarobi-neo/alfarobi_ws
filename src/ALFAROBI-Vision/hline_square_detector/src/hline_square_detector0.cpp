...
HLineSquare::HLineSquare()
    :nh_(ros::this_node::getName()),
      ...
      it_bf_sub_(it_.subscribe("ball_ref", 1, &HLineSquare::ballRefCallback, this)),
...
void HLineSquare::publishLocalizationUtils(const cv::Mat &_segmented_white,
...
//line horizontal
    ...
    cv::Mat img_hsv, img_trs,img_hasil,img_ori;

    cv::bitwise_and(segmented_white,field_contour,img_trs);
    //cv::dilate(img_trs,img_trs,cv::getStructuringElement(cv::MORPH_ELLIPSE,cv::Size(3,3)));
    cv::Mat horizontal = img_trs.clone();

    int horizontal_size = horizontal.cols / 8; //semakin besar pembagi, semakin kecil panjangnya
...
//      cout<<area<<endl;
    ...
    cv::Mat horizontal_final,field_contour_;
    cv::cvtColor(field_contour,field_contour_,cv::COLOR_GRAY2BGR);
    cv::bitwise_and(field_contour_,in_img_,horizontal_final);
//    cv::cvtColor(horizontal_final,horizontal_final,cv::COLOR_BGR2RGB);
    cv::drawContours(horizontal_final,hull_final,0,cv::Scalar(255,0,0,0.4),CV_FILLED);

    cv::Moments m;
    cv::Point hline_p;
    if(hull_final.size()) {
        m = cv::moments(hull_final[0],false);
        hline_p = cv::Point(m.m10/m.m00, m.m01/m.m00);
        cv::circle( horizontal_final, hline_p, 1, cv::Scalar(0,255,0), 3, 8, 0 );
    }
    cv::imshow ("a", horizontal_final); 
    cv::imshow ("wa", thresh_image); 
//    hline_pos_.x = hline_p.x;
//    hline_pos_.y = hline_p.y;
//    hline_pos_pub_.publish(hline_pos_);


//    cv::cvtColor(horizontal_final,horizontal_final,cv::COLOR_GRAY2RGB);
//    cv::cvtColor(in_img_,img_ori,cv::COLOR_BGR2RGB);
//    cv::bitwise_and(in_img_,horizontal_final,img_hasil);
//    cv::imshow("ori",in_img_);
//    cv::imshow("horizontal",horizontal);
//    cv::imshow("hasil",horizontal_final);
...
//==========================================================
    ...
    case 3:{
        setOutputImage(horizontal_final);break;
       //setOutputImage(horizontal_final);break;
    }
    default:
