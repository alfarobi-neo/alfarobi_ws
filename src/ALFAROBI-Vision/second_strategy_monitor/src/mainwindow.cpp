#include "second_strategy_monitor/mainwindow.h"
...
MainWindow::MainWindow(QWidget *parent) :
    ...
    icon_path << ros::package::getPath("second_strategy_monitor") << "/icon/logo_alfarobi.png";
    QIcon icon_file;
    icon_file.addFile(icon_path.str().c_str());
    this->setWindowIcon(icon_file);

    std::stringstream style_path;
    style_path << ros::package::getPath("second_strategy_monitor") << "/style/vision_style.qss";
...
void MainWindow::browseLUT(){
    QString LUT_path = QFileDialog::getOpenFileName(this,"Search LUT file...",ros::package::getPath("v9_ball_pillar_detector").c_str(),"XML File(*.xml)");
...
void MainWindow::updateParam1Value(int _value){
    ...
    ros::service::call("/v9_ball_pillar_detector_node/set_parameters",srv_req_,srv_resp_);
...
void MainWindow::loadConfig(){
    ...
    try{
        config_file = YAML::LoadFile(ros::package::getPath("v9_ball_pillar_detector") + "/config/saved_config.yaml");
    }catch(const std::exception &e){
        ROS_ERROR("[second_strategy_monitor] Problem while opening config file !!!");
    }
