#include "second_strategy_monitor/frameprocess.h"

FrameProcess::FrameProcess(QObject *parent):QThread(parent),
    nh_(ros::this_node::getName()),it_(this->nh_){
    running=false;

    it_subs_ = it_.subscribe("/v9_ball_pillar_detector_node/image_out",1,&FrameProcess::imgCallback,this);
...
void FrameProcess::imgCallback(const sensor_msgs::ImageConstPtr &_msg){
    ...
    try{
        ...
        ROS_ERROR("[second_strategy_monitor] cv_bridge exception: %s", e.what());
    }
