#include "pillar_monitor/mainwindow.h"
...
MainWindow::MainWindow(QWidget *parent) :
    ...
    icon_path << ros::package::getPath("pillar_monitor") << "/icon/logo_alfarobi.png";
    QIcon icon_file;
    icon_file.addFile(icon_path.str().c_str());
    this->setWindowIcon(icon_file);

    std::stringstream style_path;
    style_path << ros::package::getPath("pillar_monitor") << "/style/vision_style.qss";
...
void MainWindow::settingWidgets(){
    ...
    vision_mode_rb1->setText(tr("Pillar"));
    ...
    lut_dir_display->setText(tr("/home/gev/LUT.xml"));
...
//    lut_dir_display->setFixedSize(lut_dir_display->minimumSizeHint());
    ...
    param1_layout->addItem(new QSpacerItem(0,0,QSizePolicy::Expanding,QSizePolicy::Expanding),8,2);

    param1_widget = new QWidget;
    param1_widget->setLayout(param1_layout);

    param_tab = new QTabWidget;
    param_tab->addTab(param1_widget,"Pillar");
...
void MainWindow::browseLUT(){
    QString LUT_path = QFileDialog::getOpenFileName(this,"Search LUT file...",ros::package::getPath("v10_pillar_detector").c_str(),"XML File(*.xml)");
...
void MainWindow::updateParam1Value(int _value){
    ...
    ros::service::call("/v10_pillar_detector_node/set_parameters",srv_req_,srv_resp_);
...
void MainWindow::loadConfig(){
    ...
    try{
        config_file = YAML::LoadFile(ros::package::getPath("v10_pillar_detector") + "/config/saved_config.yaml");
    }catch(const std::exception &e){
        ROS_ERROR("[pillar_monitor] Problem while opening config file !!!");
    }
