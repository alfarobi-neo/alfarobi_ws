#include "monitor_hline_square/mainwindow.h"
...
MainWindow::MainWindow(QWidget *parent) :
    ...
    icon_path << ros::package::getPath("monitor_hline_square") << "/icon/logo_alfarobi.png";
    QIcon icon_file;
    icon_file.addFile(icon_path.str().c_str());
    this->setWindowIcon(icon_file);

    std::stringstream style_path;
    style_path << ros::package::getPath("monitor_hline_square") << "/style/vision_style.qss";
    ...
    LUT_pub_ = nh_.advertise<vision_utils::LUT>("mLUT_data",10);
    frame_mode_pub_ = nh_.advertise<std_msgs::Int8>("mframe_mode",10);
    save_param_pub_ = nh_.advertise<std_msgs::Empty>("msave_param",10);

    it_pub_ = it_.advertise("mball_ref",10);
...
void MainWindow::browseLUT(){
    QString LUT_path = QFileDialog::getOpenFileName(this,"Search LUT file...",ros::package::getPath("hline_square_detector").c_str(),"XML File(*.xml)");
...
void MainWindow::updateParam1Value(int _value){
    ...
    ros::service::call("/monitor_hline_square_node/set_parameters",srv_req_,srv_resp_);
...
void MainWindow::loadConfig(){
    ...
    try{
        config_file = YAML::LoadFile(ros::package::getPath("hline_square_detector") + "/config/saved_config.yaml");
