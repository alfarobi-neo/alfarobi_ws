#include "v12_goalpost_white_monitor/mainwindow.h"
...
MainWindow::MainWindow(QWidget *parent) :
    ...
    icon_path << ros::package::getPath("v12_goalpost_white_monitor") << "/icon/logo_alfarobi.png";
    QIcon icon_file;
    icon_file.addFile(icon_path.str().c_str());
    this->setWindowIcon(icon_file);

    std::stringstream style_path;
    style_path << ros::package::getPath("v12_goalpost_white_monitor") << "/style/vision_style.qss";
    ...
    this->setWindowTitle(tr("Goalpost Monitor"));
    ...
    it_pub_ = it_.advertise("goalpost_ref",10);
...
void MainWindow::settingWidgets(){
    ...
    param1_layout->addWidget(canny1_label_,   2,0,1,1);
    param1_layout->addWidget(canny1_slider_,  3,0,1,2);
    param1_layout->addWidget(canny2_label_,    4,0,1,1);
    param1_layout->addWidget(canny2_slider_,   5,0,1,2);
    param1_layout->addWidget(intersections_label_,    6,0,1,1);
    param1_layout->addWidget(intersections_slider_,   7,0,1,2);    
    param1_layout->addWidget(threshold_corner_label_,    8,0,1,1);
    param1_layout->addWidget(threshold_corner_slider_,   9,0,1,2);
...
void MainWindow::settingActions(){
    ...
    connect(canny1_slider_,SIGNAL(valueChanged(int)),this,SLOT(updateParam1Value(int)));
    connect(canny2_slider_,SIGNAL(valueChanged(int)),this,SLOT(updateParam1Value(int)));
    connect(threshold_corner_slider_,SIGNAL(valueChanged(int)),this,SLOT(updateParam1Value(int)));
    connect(intersections_slider_,SIGNAL(valueChanged(int)),this,SLOT(updateParam1Value(int)));
}
...
void MainWindow::browseLUT(){
    QString LUT_path = QFileDialog::getOpenFileName(this,"Search LUT file...",ros::package::getPath("v12_goalpost_white_detector").c_str(),"XML File(*.xml)");
...
void MainWindow::updateParam1Value(int _value){
    ...
    ros::service::call("/v12_goalpost_white_detector_node/set_parameters",srv_req_,srv_resp_);
...
void MainWindow::loadConfig(){
    ...
    try{
        config_file = YAML::LoadFile(ros::package::getPath("v12_goalpost_white_detector") + "/config/saved_config.yaml");
    }catch(const std::exception &e){
        ROS_ERROR("[goalpost_monitor] Problem while opening config file !!!");
    }
