...
Cat2Localization::Cat2Localization():nh_(ros::this_node::getName()),it_(nh_),
    // sw_sub_(nh_, "/v10_cat2_detector_node/segment_white",1),
    ...
    save_params_sub_(nh_.subscribe("/vision_monitor_node/save_param", 1, &Cat2Localization::saveParamsCb, this)),
    ...
    input_sync_.registerCallback(boost::bind(&Cat2Localization::utilsCallback, this, _1, _2, _3, _4, _5));
...
void Cat2Localization::utilsCallback(const sensor_msgs::ImageConstPtr &_sw_msg,
    ...
    try{
        ...
        ROS_ERROR("[v10_cat2_localization] cv_bridge exception: %s",e.what());
    }
...
void Cat2Localization::actionForMonitor(){
    if(params_req_){
...
void Cat2Localization::publishTipPoints(vision_utils::LineTip _tip_points){
...
void Cat2Localization::initializeFieldFeaturesData(){
...
std::vector<Points > Cat2Localization::sampleCircleCandidates(cv::Mat &_points_map, const Points &_points){
...
void Cat2Localization::getFeaturesModels(Points &_target_points,
...
#endif
int Cat2Localization::getIntersectionType(const cv::Mat &_segmented_image, const cv::Vec2f _model[2],const cv::Point &_center){
...
vision_utils::Features Cat2Localization::getLineIntersection(Vectors3 _line_models){
    // Not used yet
...
Points Cat2Localization::pointsProjection(const Points &_points, bool pos){
...
//        float distance_y = verticalDistance(offset_tilt);
        ...
        if(sqrt(distance_x*distance_x + distance_y*distance_y) < 55.0 && !pos)//ignore feature in less than 55 cm
            continue;
...
Points Cat2Localization::fastPointsProjection(const Points &_points, bool pos){
    Points projected_point;
    cv::Mat points_map = cv::Mat::zeros(POINTS_MAP_H, POINTS_MAP_W, CV_8UC1);
#ifdef DEBUG
    cv::Mat target_points_viz = cv::Mat::zeros(FRAME_HEIGHT, FRAME_WIDTH,CV_8UC1);
...
float Cat2Localization::calcShootDir(const cv::Point2f &_pos){//global ball pos
...
//    std::cout << zero_dir_area2_tl << " ; " << zero_dir_area2_br << std::endl;
    ...
    if((_pos.x > zero_dir_area1_tl.x && _pos.x < zero_dir_area1_br.x &&
        _pos.y > zero_dir_area1_tl.y && _pos.y < zero_dir_area1_br.y) ||
            (_pos.x > zero_dir_area2_tl.x && _pos.x < zero_dir_area2_br.x &&
             _pos.y > zero_dir_area2_tl.y && _pos.y < zero_dir_area2_br.y) ||
            resetting_particle_){
        return 360.0f;
    }
...
//    std::cout << gk_avoidance_area_tl << " ; " << gk_avoidance_area_br << std::endl;
    ...
    if((_pos.x >= gk_avoidance_area_tl.x && _pos.x <= gk_avoidance_area_br.x &&
        _pos.y >= gk_avoidance_area_tl.y && _pos.y <= gk_avoidance_area_br.y)){
        ...
        float target_dir = atan2(target_goal.y - _pos.y, target_goal.x - _pos.x) * Math::RAD2DEG;
...
//    if((_pos.x >= avoidance_gk_area1_tl.x && _pos.x <= avoidance_gk_area1_br.x &&
//        _pos.y >= avoidance_gk_area1_tl.y && _pos.y <= avoidance_gk_area1_br.y)){
...
//    if((_pos.x > avoidance_gk_area2_tl.x && _pos.x < avoidance_gk_area2_br.x &&
//        _pos.y > avoidance_gk_area2_tl.y && _pos.y < avoidance_gk_area2_br.y)){
//        cv::Point2f target_goal(center_goal.x, center_goal.y - GOAL_WIDTH * 0.5f);
//        float target_dir = atan2(target_goal.y - _pos.y, target_goal.x - _pos.x) * Math::RAD2DEG;
//        if(target_dir < .0f)target_dir = 360.0f + target_dir;
//        return target_dir;
...
//==============
    ...
    float diff_x1 = _pos.x - goal_post1.x;
    float diff_y1 = _pos.y - goal_post1.y;
    float diff_x2 = _pos.x - goal_post2.x;
    float diff_y2 = _pos.y - goal_post2.y;
...
//    float center_dir = (attack_dir_?CV_PI - atan2(center_goal.y - _pos.y,_pos.x - center_goal.x):
//      atan2(center_goal.y - _pos.y,center_goal.x - _pos.x))*Math::RAD2DEG;
    float center_dir = atan2(center_goal.y - _pos.y, center_goal.x - _pos.x) * Math::RAD2DEG;
...
void Cat2Localization::publishData(){
    ...
    if(square_pos_.x != -1.0 &&
        ...
        // square_pos = fastPointsProjection(square_pos, true);
    ...
    if(line_pos_.x != -1.0 &&
        ...
        // line_pos = fastPointsProjection(line_pos, true);
...
void Cat2Localization::process(){
        ...
        std::cout<<line_pos_.x<<std::endl;
        // cv::imshow("asu",cv_isg_ptr_sub_->image);
