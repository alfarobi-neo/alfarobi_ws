...
//#endif
    ...
    Points pointsProjection(const Points &_points, bool pos=false);
    Points fastPointsProjection(const Points &_points, bool pos=false);
    ...
    void loadParams(){
        ...
        try{
            params_file = YAML::LoadFile(ros::package::getPath("v10_cat2_localization") + "/config/saved_params.yaml");
        }catch(const std::exception &e){
            ROS_ERROR("[v10_cat2_localization] Unable to load params file: %s", e.what());
        }
    ...
    void saveParams(){
        ...
        std::ofstream file_out(ros::package::getPath("v10_cat2_localization") + "/config/saved_params.yaml");
    ...
    bool attack_dir_;
    float calcShootDir(const cv::Point2f &_pos);
