...
const int Projection::MIN_LINE_INLIERS = 10;
const int Projection::MIN_CIRCLE_INLIERS = 30;
const int Projection::MIN_LINE_LENGTH = 60;
const int Projection::MAX_LINE_MODEL = 10;

Projection::Projection():nh_(ros::this_node::getName()),it_(nh_),
    // sw_sub_(nh_, "/v9_pillar_detector_node/segment_white",1),
    isg_sub_(nh_,"/v9_ball_pillar_detector_node/inv_segment_green",1),
    fb_sub_(nh_,"/v9_ball_pillar_detector_node/field_boundary",1),
    ...
    //present_joint_sub_ = nh_.subscribe("/robotis/present_joint_states",100,&Projection::presentJointStateCb,this);
    #ifdef GAZEBO
    //imu_data_sub_ = nh_.subscribe("/robotis_op3/imu",1,&Projection::imuDataCb,this);
    camera_info_sub_(nh_.subscribe("/robotis_op3/camera/camera_info",1,&Projection::cameraInfoCallback,this)),
    #else
    //imu_data_sub_ = nh_.subscribe("/arduino_controller/imu",1,&Projection::imuDataCb,this);
    camera_info_sub_(nh_.subscribe("/usb_cam/camera_info", 1, &Projection::cameraInfoCallback, this)),
    #endif
    // reset_particles_sub_(nh_.subscribe("/localization_monitor_node/reset_particles", 1, &Projection::resetParticlesCb, this)),
    // save_params_pillar_sub_(nh_.subscribe("/pillar_monitor_node/save_param", 1, &Projection::saveParamsPillarCb, this)),
    pillar_pos_sub_(nh_.subscribe("/v9_ball_pillar_detector_node/pillar_pos",1,&Projection::pillarPosCb,this)),
    projected_pillar_pub_(nh_.advertise<geometry_msgs::Point>("projected_pillar",10)),
    projected_pillar_stamped_pub_(nh_.advertise<geometry_msgs::PointStamped >("stamped_projected_pillar", 10)),

    ball_pos_sub_(nh_.subscribe("/v9_ball_pillar_detector_node/ball_pos",1,&Projection::ballPosCb,this)),
    ...
    // save_params_square_sub_(nh_.subscribe("/monitor_hline_square_node/save_param", 1, &Projection::saveParamsHlineCb, this)),
    square_pos_sub_(nh_.subscribe("/hline_square_detector_node/square_pos",1,&Projection::squarePosCb,this)),
    ...
    hline_pos_sub_(nh_.subscribe("/hline_square_detector_node/hline_pos",1,&Projection::hlinePosCb,this)),
    projected_hline_pub_(nh_.advertise<geometry_msgs::Point>("projected_hline",10)),
    projected_hline_stamped_pub_(nh_.advertise<geometry_msgs::PointStamped >("stamped_projected_hline", 10)),

    goalpost_pos_sub_(nh_.subscribe("/v10_goalpost_detector_node/goalpost_pos",1,&Projection::goalpostPosCb,this)),
    projected_goalpost_pub_(nh_.advertise<geometry_msgs::Point>("projected_goalpost",10)),
    projected_goalpost_stamped_pub_(nh_.advertise<geometry_msgs::PointStamped >("stamped_projected_goalpost", 10)),
    #ifdef GAZEBO
    odometry_sub_(nh_.subscribe("/robotis/pelvis_pose", 1, &Projection::odometryCb, this)),
    #else
    odometry_sub_(nh_.subscribe("/alfarobi/odometry", 100, &Projection::odometryCb, this)),
    robot_height_sub_(nh_.subscribe("/alfarobi/robot_height", 100, &Projection::robotHeightCb, this)),
    #endif
    ...
    input_sync_.registerCallback(boost::bind(&Projection::utilsCallback, this, _1, _2, _3, _4));

    param_cb_ = boost::bind(&Projection::paramCallback, this, _1, _2);
...
//void Projection::presentJointStateCb(const sensor_msgs::JointStateConstPtr &_msg){

//}

void Projection::utilsCallback(
...
void Projection::paramCallback(v10_projection::LocalizationParamsConfig &_config, uint32_t level){
    (void)level;
    new_params_ = _config;
    params_req_ = true;
}

bool Projection::setInputImage(){
...
inline float Projection::panAngleDeviation(float _pixel_x_pos){
    return atan((2.0f * _pixel_x_pos/FRAME_WIDTH - 1.0f) * TAN_HFOV_PER2);
}

inline float Projection::tiltAngleDeviation(float _pixel_y_pos){
    return atan((2.0f * _pixel_y_pos/FRAME_HEIGHT - 1.0f) * TAN_VFOV_PER2);
}

inline float Projection::verticalDistance(float _tilt_dev){
...
inline float Projection::horizontalDistance(float _distance_y, float _offset_pan){
    return _distance_y * tan(_offset_pan);
}

void Projection::genRadialPattern(){
    for(int i = 1; i <= 10; i++){
...
//void Projection::resetParticles(Particles &_particles){
...
Points Projection::pointsProjection(const Points &_points, bool pillar){
...
Points Projection::fastPointsProjection(const Points &_points, bool pillar){
    Points projected_point;
    cv::Mat points_map = cv::Mat::zeros(POINTS_MAP_H, POINTS_MAP_W, CV_8UC1);
#ifdef DEBUG
    cv::Mat target_points_viz = cv::Mat::zeros(FRAME_HEIGHT, FRAME_WIDTH,CV_8UC1);
...
inline void Projection::arrangeTargetPoints(Points &_target_points){
...
float Projection::calcShootDir(const cv::Point2f &_pillar_pos){//global pillar pos
...
void Projection::publishData(){
    ...
    geometry_msgs::PointStamped proj_hline_stamped_msg;
    proj_hline_stamped_msg.header.stamp = this->stamp_;
    proj_hline_stamped_msg.header.seq++;
    geometry_msgs::Point hline_pos_msg;
    hline_pos_msg.z = -1.0;
    proj_hline_stamped_msg.point = hline_pos_msg;

    if(hline_pos_.x != -1.0 &&
        hline_pos_.y != -1.0){
        Points hline_pos;
        hline_pos.emplace_back(cv::Point(hline_pos_.x,hline_pos_.y));
        hline_pos = pointsProjection(hline_pos, true);

        if(hline_pos.size() > 0){

            float c_t = cos(robot_state_.theta * Math::DEG2RAD);
            float s_t = sin(robot_state_.theta * Math::DEG2RAD);
            float shoot_dir = calcShootDir(cv::Point2f(robot_state_.x + hline_pos.front().y*c_t - hline_pos.front().x*s_t,
                                     robot_state_.y + hline_pos.front().y*s_t + hline_pos.front().x*c_t));

            hline_pos_msg.x = hline_pos.front().y;
            hline_pos_msg.y = -hline_pos.front().x;
            hline_pos_msg.z = shoot_dir;
            projected_hline_pub_.publish(hline_pos_msg);

            proj_hline_stamped_msg.point.x = hline_pos.front().x;
            proj_hline_stamped_msg.point.y = hline_pos.front().y;
            proj_hline_stamped_msg.point.z = shoot_dir;
        }
    }
    ...
    projected_hline_stamped_pub_.publish(proj_hline_stamped_msg);
}

void Projection::sampleMotionModelOdometry(
...
void Projection::measurementModel(Particles &_particles_state,
