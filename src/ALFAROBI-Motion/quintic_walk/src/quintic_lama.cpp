...
void QuinticWalk::process(std::map<std::string, robotis_framework::Dynamixel *> dxls, std::map<std::string, double> sensors)
{
    ...
    for(int idx = 0; idx < 12; idx++)
    {
        ...
        // ////std::cout<<"current_joint_pos["<<idx<<"]"<<current_joint_pos(idx)*RADIAN2DEGREE<<std::endl;
    }
    ...
    {
        // std::cout<<"COMDSPpitch: "<<COMDSPrpy(1)*180/M_PI<<" degrees"<<std::endl;
            // std::cout<<"gyroPitch: "<<m_imuGyr(1)*180/M_PI<<" deg/s"<<std::endl;
         ...
         state_it != result_.end(); state_it++)
    {
        ...
        // std::cout<<"op3_kd: "<<joint_name<<" goal pos: "<<result_[joint_name]->goal_position_<<std::endl;
    }
...
void QuinticWalk::fuzzyQuintic()
{
    ...
    ANGLE.input = COMDSPrpy(1)*180/PI-mbc.pitchRef;
    ...
    // std::cout<<"Angle : "<<ANGLE.input<<"\t KP : "<<gainFuzzy[0][0]<<"    KD : "<<gainFuzzy[0][1]<<std::endl;
}
...
void QuinticWalk::calculateWalking()
{
    ...
    */
    ...
    ////std::cout<<"X MOVE\t"<< _orders.x()<<std::endl;
    _orders /=10;
...
{
    joint_goals[12]=0;
    joint_goals[13]=35*DEGREE2RADIAN;
    joint_goals[14]=-120*DEGREE2RADIAN;
    joint_goals[15]=0;
    joint_goals[16]=-35*DEGREE2RADIAN;
...
void QuinticWalk::calcCapturePoint()
{
    Eigen::ArrayXd capturePoint(4);
    ...
    integralAccel += (accel_COM.x()*dT); //bener gak ya x

    // capturePoint(0) -> x_dot . sqrt(z_com / g)

    //                            z_com . omega_com
    // capturePoint(1) -> x_com + -------------------
    //                               omega_poros
    ...
    // x_com  = tan2(angle) . z_com

    //                                            z_com . omega_com
    // capturePoint(1) -> atan2(angle) . z_com + -------------------
    //                                                angle_dot

    //                                            z_com . omega_com
    // capturePoint(2) -> atan2(angle) . z_com + -------------------
    //                                               integral_acc

    // capturePoint(3) -> atan2(angle) . z_com + z_com

    if(IndexSupport==2) //SSP Kanan
    {
        capturePoint(0) = m_imuGyr.y() * COM_DSP(2)  * sqrt(COM_DSP(2) / G_CONSTANT);
        capturePoint(1) = (tan(accel_COM.x()) * COM_DSP(2)) + (COM_DSP(2) * m_imuGyr.y() / orientation_y_dt);
        capturePoint(2) = (tan(accel_COM.x()) * COM_DSP(2)) + (COM_DSP(2) * m_imuGyr.y() / integralAccel);
        capturePoint(3) = (tan(accel_COM.x()) * COM_DSP(2)) + COM_DSP(2);
    }
    ...
    densisMsgs.CP_0 = capturePoint(0);
    densisMsgs.CP_1 = capturePoint(1);
    densisMsgs.CP_2 = capturePoint(2);
    densisMsgs.CP_3 = capturePoint(3);

    orientation_y_dt = accel_COM.x() / dT;
}
...
void QuinticWalk::simpleCapturePoint()
{
  ...
  if ((orientation_y_capture > (7 * DEGREE2RADIAN) )  && firstActive && (ros::Time::now().toSec() - timeStep > 0.5))
  {  
  ...
  if (supportCount == 2)
  {
      walking_param_.XMove = lastXMove;
      firstActive = true;
      countCapture = false;
  }
  ...
  std::cout<<"<<<<<==========LASXMove=================>>>>>>>\t"<<lastXMove<<std::endl;
}
