...
struct JointConfig
{
    ...
    JointConfig(double x, double y, double z)
    {
        setPos(x, y, z);
    }
    ...
    inline JointConfig operator+(const JointConfig& v) const {return JointConfig(x+v.x, y+v.y, z+v.z);}
    inline JointConfig operator-() const {return JointConfig(-x, -y, -z);}
    inline JointConfig operator-(const JointConfig& v) const {return JointConfig(x-v.x, y-v.y, z-v.z);}
...
struct AbstractArmPose
{
    ...
    AbstractArmPose(const Vector3d& armAngle, double armExtension)
    {
        this->armAngle = armAngle;
        this->armExtension = armExtension;
    }

    inline AbstractArmPose operator+(const AbstractArmPose& v) const {return AbstractArmPose(armAngle+v.armAngle, armExtension+v.armExtension);}
    inline AbstractArmPose operator-() const {return AbstractArmPose(-armAngle, -armExtension);}
    inline AbstractArmPose operator-(const AbstractArmPose& v) const {return AbstractArmPose(armAngle-v.armAngle, armExtension-v.armExtension);}
...
struct InverseArmPose
{
    ...
    InverseArmPose(const Vector3d& handPosition)
    {
        this->handPosition = handPosition;
    }

    inline InverseArmPose operator+(const InverseArmPose& v) const {return InverseArmPose(handPosition+v.handPosition);}
    inline InverseArmPose operator-() const {return InverseArmPose(-handPosition);}
    inline InverseArmPose operator-(const InverseArmPose& v) const {return InverseArmPose(handPosition-v.handPosition);}
...
struct AbstractLegPose
{
    ...
    AbstractLegPose(const Vector3d& legAngle, double legExtension, const Vector2d& footAngle)
    {
    ...
    inline AbstractLegPose operator+(const AbstractLegPose& v) const {return AbstractLegPose(legAngle+v.legAngle, legExtension+v.legExtension, footAngle+v.footAngle);}
    inline AbstractLegPose operator-() const {return AbstractLegPose(-legAngle, -legExtension, -footAngle);}
    inline AbstractLegPose operator-(const AbstractLegPose& v) const {return AbstractLegPose(legAngle-v.legAngle, legExtension-v.legExtension, footAngle-v.footAngle);}
...
struct InverseLegPose
{
    ...
    InverseLegPose(const Vector3d& footPosition, const Vector3d& footAngle)
    {
        this->footPosition = footPosition;
        this->footAngle = footAngle;
    }

    inline InverseLegPose operator+(const InverseLegPose& v) const {return InverseLegPose(footPosition+v.footPosition, footAngle+v.footAngle);}
    inline InverseLegPose operator-() const {return InverseLegPose(-footPosition, -footAngle);}
    inline InverseLegPose operator-(const InverseLegPose& v) const {return InverseLegPose(footPosition-v.footPosition, footAngle-v.footAngle);}
